...
sh ./sub_entrypoint.sh

# Configuracao para inicio da replica
bucardo -h db -U bucardo add dbgroup idmed_dbgroup main:source idmedCentral:target
bucardo -h db -U bucardo add sync idmed_sync dbgroup=idmed_dbgroup relgroup=idmed_relgroup conflict_strategy=bucardo_latest_all_tables onetimecopy=0 autokick=1
