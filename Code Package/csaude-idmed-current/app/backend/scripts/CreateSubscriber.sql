...
BEGIN
    ...
    END IF;
    	EXECUTE 'CREATE SUBSCRIPTION sub_'||clinicRecord.clinic_id||' CONNECTION ''host='||target_db_host||' port='||target_db_port||' user='||target_db_user||' dbname='||target_db_name||' sslmode=allow '' PUBLICATION pub_'||clinicRecord.clinic_id ||' WITH (create_slot = false, copy_data = false)';
