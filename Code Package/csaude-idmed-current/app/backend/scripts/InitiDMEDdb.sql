...
WITH entrada AS (
 ...
 SELECT entrada.event_year,
    ...
    entrada.negativeadjustment,ALTER TABLE IF EXISTS form ADD COLUMN IF NOT EXISTS unit character varying(255) NOT NULL DEFAULT '';
    entrada.losses,
    entrada.clinic_id,
    entrada.code,
    entrada.stock,
...
( WITH inventario AS (
         SELECT EXTRACT(year FROM i.end_date) AS event_year,
            ...
            sum(
                CASE
                    WHEN sot.code::text = 'AJUSTE_POSETIVO'::text THEN ceALTER TABLE IF EXISTS form ADD COLUMN IF NOT EXISTS unit character varying(255) NOT NULL DEFAULT '';il(sa.adjusted_value::double precision)
...
AS WITH entrada AS (
 ...
 SELECT entrada.drug_id,
    ...
    entrada.code,
    entrada.stock,
    entrada.max_datedocker-compose down && 
   FROM entrada
UNION
( WITH saida AS (
         SELECT pd.drug_id,
...
CREATE OR REPLACE VIEW public.patient_last_3_visits_screening_vw
 ...
 WITH ranked_visits AS (
         SELECT pv.id,
            ...
            pv.patient_id,
            pv.visit_date,
            pv.clinic_id,ALTER TABLE IF EXISTS form ADD COLUMN IF NOT EXISTS unit character varying(255) NOT NULL DEFAULT '';
            pv.creation_date,
            pv.origin,
...
-- 1.5.0
ALTER TABLE IF EXISTS provincial_server ADD COLUMN IF NOT EXISTS dbname character varying(255) NOT NULL DEFAULT '';
ALTER TABLE IF EXISTS public.clinical_service_clinic_sector ADD CONSTRAINT clinical_service_clinic_sector_pkey PRIMARY KEY (clinical_service_clinic_sectors_id, clinic_sector_id);
...
-- 1.6.0
ALTER TABLE IF EXISTS episode ADD COLUMN IF NOT EXISTS resident_in_country boolean NOT NULL DEFAULT true;
ALTER TABLE openmrs_error_log ALTER COLUMN json_request DROP NOT NULL;
